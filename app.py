...
#st.title("Sentiment-Based Task Recommender 🧠")
st.markdown("""<p style="font-size: 38px; font-weight: bold;"> Sentiment-Based Task Recommender 🧠 </p>""", unsafe_allow_html=True)
#st.write("Get personalized task suggestions based on how you feel.")
st.markdown("""<p style="font-size: 20px; font-weight: light;"> I am here to help you with task recommendations based on your mood! </p>""", unsafe_allow_html=True)

# Mood Input
user_mood = st.text_area(label = "", placeholder  = "How are you feeling today?")


if st.button("Get Task Recommendations", type = "secondary"):
        ...
        if recommended:
            ...
            for task in recommended:
                ...
                if task['priority'] == "Low":
                    st.markdown(f"- **{task['task']}**   _(Priority: :blue-badge[{task['priority']}])_")
